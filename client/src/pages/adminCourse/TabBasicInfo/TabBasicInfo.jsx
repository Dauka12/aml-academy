...
const TabBasicInfo = ({
  ...
  price: initPrice,
  image: initImage,
  typeOfStudy: initType,
  academicHours: initHours,
}) => {
  ...
  const [defImage, setDefImage] = useState(
    !initImage || initImage === base64Course
  );
  const [image, setImage] = useState(
    initImage || (defImage ? base64Course : "")
  );
    ...
    if (id !== 0) {
      ...
      axios
        ...
        .then((res) => {
          ...
          setCategory(
            res.data.courseCategory ? res.data.courseCategory.category_id : 0
          );
          ...
          setAcademicHours(res.data.academicHours);
          console.log("res.data.academicHours:", res.data.academicHours);
  ...
  const validateForm = () => {
    ...
    if (academicHours < 1)
      errors.academicHours = "Академические часы должны быть больше 0";
    if (!typeOfStudy) errors.typeOfStudy = "Пожалуйста, выберите тип обучения";
    if (!image && !defImage)
      errors.image =
        "Пожалуйста, загрузите изображение или используйте по умолчанию";
